...
class UsersController{
    static async findAll(params){
        try{
            ...
            if(totalData == 0) throw {name: "noDataFound", message: `There is no user data found with current specific parameters`}; 
    ...
    static async findSingle(id){
        try{
            let sqlQuery = `SELECT * FROM ${tableName}
                            WHERE id=${id}`;
            const result = await pool.query(sqlQuery);
            if(result.rowCount == 0) throw {name: "noDataFound", message: `There is no user with id ${id}`};    
            return result.rows;
        }catch(err){
            throw err;
        }
